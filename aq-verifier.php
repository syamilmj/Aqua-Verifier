...
if ( ! class_exists( 'AQ_Verifier' ) ) {
	...
	class AQ_Verifier {
		...
		function register_settings() {
			...
			add_settings_field( 'marketplace_username', 'Market Username', array( $this, 'settings_field_input' ), $slug, $slug, array(
				                                          'id'   => 'marketplace_username',
				                                          'desc' => __( 'Case sensitive', 'a10e_av' ),
			                                          ) );

			add_settings_field( 'api_key', 'API Key', array( $this, 'settings_field_input' ), $slug, $slug, array(
				                             'id'   => 'api_key',
				                             'desc' => __( 'More info about ', 'a10e' ) . '<a href="http://themeforest.net/help/api">Envato API</a>',
			                             ) );

			add_settings_field( 'custom_style', 'Custom Styling', array( $this, 'settings_field_textarea' ), $slug, $slug, array(
				                                  'id'   => 'custom_style',
				                                  'desc' => __( 'Add custom inline styling to the registration page', 'a10e_av' ),
			                                  ) );

			add_settings_field( 'disable_username', 'Disable Username input', array( $this, 'settings_field_checkbox' ), $slug, $slug, array(
				                                      'id'   => 'disable_username',
				                                      'desc' => __( 'Disable the username field and use only the purchase code', 'a10e_av' ),
			                                      ) );

			add_settings_field( 'display_credit', 'Display "Powered by"', array( $this, 'settings_field_checkbox' ), $slug, $slug, array(
				                                    'id'   => 'display_credit',
		...
		function view_verification_form( $errors = '' ) {
			...
			<form name="registerform" id="registerform" action="<?php echo esc_url( site_url( 'wp-login.php?action=register', 'login_post' ) ); ?>"
				...
				<p class="submit"><input type="submit" name="wp-submit" id="wp-submit" class="button button-primary button-large"
				                         value="<?php esc_attr_e( 'Verify' ); ?>" tabindex="100"/></p>
			</form>
